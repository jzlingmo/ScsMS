...
class Connection(object):
    ...
    def execute_lastrowid(self, query, parameters=None):
        """Executes the given query, returning the lastrowid from the query."""
        cursor = self._cursor()
        try:
            self._execute(cursor, query, parameters)
    ...
    def execute_rowcount(self, query, parameters=None):
    ...
    def update_by_dict(self, table_name, dict_row, dict_where):
        cursor = self._cursor()
        cursor.execute("describe %s" % table_name)
        allowed_keys = set(row[0] for row in cursor.fetchall())
        keys = allowed_keys.intersection(dict_row)

        if len(dict_row) > len(keys):
            unknown_keys = set(dict_row) - allowed_keys

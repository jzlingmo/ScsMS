...
class WoaiduBookFile(FilePipeline):
    ...
    def get_media_requests(self, item, info):
        ...
        if item.get('book_download'):
            ...
            self.item_download[item['url']] = downloadfile_urls[1:]
    ...
    def item_completed(self, results, item, info):
        ...
        if bookfile_path_url:
            ...
            if self.item_download[item['url']]:
                next = list_first_item(self.item_download[item['url']])
                self.item_download[item['url']] = self.item_download[item['url']][1:]
                return Request(next)
            else:
                return item
